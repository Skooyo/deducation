...
const BountyCard = ({ bounty }: BountyCardProps) => {
  ...
  return (
    <>
      <div
        className="
                flex h-40 w-full border-2 border-s3 rounded-lg
                hover:cursor-pointer transform transition duration-300 hover:translate-x-2 hover:shadow-lg bg-gradient-to-br from-s5/50 to-s2/100"
      ...
      >
        ...
        {/* text */}
        <div className="flex flex-col justify-between w-full h-[160px] p-4 text-p5">
          <h1 className="text-xl font-semibold line-clamp-1 text-p4">
            {bounty.title}
          </h1>
          ...
          <div className="pt-3 flex flex-wrap justify-between">
            <p className="text-red-500">{`Due in ${calculateDueDate(
              bounty.dueDate
            )} days (${getFormattedDate(bounty.dueDate)})`}</p>
